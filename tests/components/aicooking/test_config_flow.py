"""Test the Aicooking config flow."""
...
from . import BBQ_SERVICE_INFO, NOT_AICOOKING_SERVICE_INFO
...
async def test_async_step_bluetooth_valid_device(hass):
    ...
    result = await hass.config_entries.flow.async_init(
        ...
        data=BBQ_SERVICE_INFO,
    )
    assert result["type"] == FlowResultType.FORM
    assert result["step_id"] == "bluetooth_confirm"
    with patch(
        "homeassistant.components.aicooking.async_setup_entry", return_value=True
    ):
...
@pytest.mark.skip(reason="Temporarily disabled - TODO")
async def test_async_step_bluetooth_not_aicooking(hass):
...
async def test_async_step_user_with_found_devices(hass):
    ...
    with patch(
        "homeassistant.components.aicooking.config_flow.async_discovered_service_info",
        return_value=[BBQ_SERVICE_INFO],
    ):
        result = await hass.config_entries.flow.async_init(
            DOMAIN,
            context={"source": config_entries.SOURCE_USER},
