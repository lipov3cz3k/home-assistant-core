...
class LoginFlow(data_entry_flow.FlowHandler):
    ...
    async def async_step_mfa(
        ...
        data = None
        if (user_input is None or errors) and hasattr(
            auth_module, "async_get_login_mfa_additional_data"
        ):
            data = await auth_module.async_get_login_mfa_additional_data(self.user.id)
